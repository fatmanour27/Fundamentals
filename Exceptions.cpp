...
void BadAllocation()
{
	try
	{
		...
		else if (size <= 0)  //when the user input size < 0 the specific message"Invalid Array size" will not thrown
			                //and the derived class 'runtime_error' will throw its default message in this case
		{
...
void UnderFlow() 
{
	try
	{
		...
		if (value < FLT_MIN && value != 0.0 )    //A negative value "alone" is not necessarily an underflow but 
			                                    //a negative floating value considered as very small floating point
		{
